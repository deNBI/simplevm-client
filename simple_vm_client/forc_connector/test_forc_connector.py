...
from simple_vm_client.ttypes import BackendNotFoundException, DefaultException
...
class TestForcConnector(unittest.TestCase):
    ...
    def test_init(self, mock_template, mock_redis, mock_connection_pool):
        ...
        mock_redis.assert_called_with(connection_pool=mock_connection_pool.return_value, charset="utf-8")
    ...
    def test_connect_to_redis(self, mock_logger_error, mock_logger_info, mock_redis, mock_redis_pool):
        self.forc_connector.connect_to_redis()
        mock_redis_pool.assert_any_call(host=self.forc_connector.REDIS_HOST, port=self.forc_connector.REDIS_PORT)
        mock_redis.asser_called_once_with(connection_pool=self.forc_connector.redis_pool, charset="utf-8")
    ...
    def test_get_users_from_backend(self, mock_get):
        ...
        mock_get.assert_called_once_with(get_url, timeout=(30, 30), headers={"X-API-KEY": self.forc_connector.FORC_API_KEY}, verify=True)
    ...
    def test_add_user_to_backend(self, mock_post):
        ...
        result = self.forc_connector.add_user_to_backend(backend_id="backend_id", user_id="user_id")
    ...
    def test_add_user_to_backend_exception(self, mock_post):
        ...
        with self.assertRaises(BackendNotFoundException):
             self.forc_connector.add_user_to_backend(backend_id="backend_id", user_id="user_id")
    ...
    def test_set_vm_wait_for_playbook(self):
        ...
        private_key = "priv"
        name = "name"
        self.forc_connector.set_vm_wait_for_playbook(openstack_id=openstack_id, private_key=private_key, name=name)
        self.forc_connector.redis_connection.hset.assert_called_once_with(name=openstack_id,
                                                                          mapping=dict(
                                                                              key=private_key,
                                                                              name=name,
    ...
    def test_get_playbook_status(self):
        ...
        self.forc_connector.redis_connection.hget.return_value=VmTaskStates.PREPARE_PLAYBOOK_BUILD.value.encode("utf-8")
        ...
        self.forc_connector.redis_connection.hget.return_value=VmTaskStates.BUILD_PLAYBOOK.value.encode("utf-8")
        result=self.forc_connector.get_playbook_status(server=fake_server)
        self.assertEqual(result.task_state,VmTaskStates.BUILD_PLAYBOOK.value)
        self.forc_connector.redis_connection.hget.return_value=VmTaskStates.PLAYBOOK_FAILED.value.encode("utf-8")
        result=self.forc_connector.get_playbook_status(server=fake_server)
        self.assertEqual(result.task_state,VmTaskStates.PLAYBOOK_FAILED.value)
        self.forc_connector.redis_connection.hget.return_value=VmTaskStates.PLAYBOOK_SUCCESSFUL.value.encode("utf-8")
        result=self.forc_connector.get_playbook_status(server=fake_server)
    ...
    def test_create_and_deploy_playbook(self,mock_playbook):
        ...
        res=self.forc_connector.create_and_deploy_playbook(public_key=key,research_environment_template="vscode",create_only_backend=False,conda_packages=[],apt_packages=[],openstack_id=openstack_id,port=80,ip="192.168.0.1",cloud_site="Bielefeld",base_url="base_url")
        self.forc_connector.redis_connection.hset.assert_called_once_with(openstack_id,"status",VmTaskStates.BUILD_PLAYBOOK.value)
